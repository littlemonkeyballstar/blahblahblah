...
def build_guaranteed_home_featured(lectures: list[dict]) -> list[dict]:
    """Resolve always-included slideshow entries from GUARANTEED_HOME_FEATURED_STEMS."""
    by_stem = {norm(Path(lec["archive"]).stem): lec for lec in lectures}
    guaranteed: list[dict] = []
    for stem in GUARANTEED_HOME_FEATURED_STEMS:
        lec = by_stem.get(norm(stem))
        if not lec:
            print(f"Warning: guaranteed featured lecture not found: {stem}")
            continue
        guaranteed.append({
            "id": lec["id"],
            "title": lec["title"],
...
def write_catalog_outputs(
    ...
    with open(home_path, "w", encoding="utf-8") as handle:
        handle.write("/* Auto-generated — run generate-catalog.py to refresh */\n")
        handle.write("const HOME_FEATURED_GUARANTEED = ")
        json.dump(guaranteed_featured, handle, ensure_ascii=False, indent=2)
        handle.write(";\n\nconst HOME_FEATURED = ")
...
def main():
    ...
    guaranteed_featured = build_guaranteed_home_featured(lectures)
    catalog_version = __import__("datetime").date.today().strftime("%Y%m%d")
    write_catalog_outputs(lectures, cat_meta, featured_pool, guaranteed_featured, catalog_version)
